...
long long int calculatePopulations(int year, int child)
{
    ...
    long long int newChild, deathPopu = 0;
    long long int updatedPopu = CURRENT_POPULATIONS;
    int y = year;

    int index = STARTING_COUPLE_YEAR / 5;
    while(y >= 5) {
        // half of the total people
        updatedPopu += (populations[index].popu / 2) * child;
        deathPopu += ((updatedPopu * DEATH_RATE) / 1000) * 5;
    ...
    if (y) {
        updatedPopu += ((y / 5) * (populations[index].popu / 2)) * child;
        deathPopu += deathPopu += ((updatedPopu * DEATH_RATE) / 10000) * y;
    }
